...
module.exports = function() {
  ...
  this.Given(/^I am on the "([^"]*)" page$/, function (location) {
    browser.url(baseUrl + url[location] + cacheBust);
  });

  this.Then(/^I fill in the "([^"]*)" field with "([^"]*)"$/, function (selector, value) {
    client.setValue(selectorMapping[selector], value);
  });
  ...
  this.Then(/^I should see "([^"]*)" on "([^"]*)"$/, function (text, selector) {
    client.waitForExist(selectorMapping[selector], 3000);
    expect(client.getText(selectorMapping[selector])).toEqual(text);
  });
